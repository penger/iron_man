...
public class MysqlData2ES {

    private static final Logger logger = LogManager.getLogger(MysqlData2ES.class);

    public static void main(String[] args) {
        try {
            long startTime = System.currentTimeMillis();
            JdbcComponent.JdbcParam jdbcParam = JdbcFactory.getJdbc("mymysql");
            String indexName = "process_definition";
//            String sql = "select id,name,address ,province ,phone_number ,ssn , birthday, email  from person ";
            String sql = "select id, project_id ,description ,name as newname  from process_definition ";
            EsComponent.EsParam esParam = EsComponent.EsParam.builder()
                    .username("elastic")
                    .password("elastic")
                    .port(9200)
                    .hostList(Arrays.asList("192.168.1.123")).build();
            Integer batchSize = 100;
            writeJdbcDataToES(esParam,jdbcParam, indexName, sql,batchSize);
    ...
    /**
     */
    private static void writeJdbcDataToES(EsComponent.EsParam esParam , JdbcComponent.JdbcParam jdbcParam, String indexName, String sql, Integer batchSize) {
